...
#if defined(NDEBUG)
# error "BitcoinHasHCore cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xf79ef3a7a2672bb6062f6ac7ef276fc08429c71baddb8014fb960258e5415149");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // BitcoinHasHCore: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "BitcoinHasHCore Signed Message:\n";
...
{
    // BitcoinHasHCore: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 9000 * COIN;

     if(nHeight == 1)
     {
        nSubsidy = 4800000000 * COIN;
     }
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // BitcoinHasHCore: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // BitcoinHasHCore: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xf79ef3a7a2672bb6062f6ac7ef276fc08429c71baddb8014fb960258e5415149");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1437859564, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "25/Jul/2015 Bitcoin HasH Core 2015";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 9000 * COIN;
        ...
        assert(block.hashMerkleRoot == uint256("0x6bb3e3923066f0264595db4bc235ceaea70850391cd72f453fea5b69e2904640"));
...
unsigned char pchMessageStart[4] = { 0xc3, 0xd2, 0xd1, 0xbd }; // BitcoinHasHCore: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BitcoinHasHCoreMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BitcoinHasHCoreMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BitcoinHasHCoreMiner(CWallet *pwallet)
{
    printf("BitcoinHasHCoreMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bitcoinhashcore-miner");
    ...
    try { loop {
        ...
        printf("Running BitcoinHasHCoreMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BitcoinHasHCoreMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BitcoinHasHCoreMiner, pwallet));
}
